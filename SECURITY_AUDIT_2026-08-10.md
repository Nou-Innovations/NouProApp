...
| `MOB-14` | 🟠 | Full user profile (email, phone, address, privacy settings) persisted unencrypted in AsyncStorage | `src/shared/store/profileStore.ts:100` | C1 | **FIXED** (2026-08-10) |
| `MOB-18` | 🟠 | The **release** build type is signed with the **debug keystore**. EAS normally injects real credentials over this, so it is probably not shipping — but a local `./gradlew assembleRelease` would produce a debug-signed artifact | `android/app/build.gradle:113` | — | **OWNER** — confirm with `eas credentials` |
| `MOB-19` | 🟡 | `aps-environment: development` in the iOS entitlements. EAS usually rewrites this from the provisioning profile; if it does not, **push notifications silently fail in production** | `ios/NouPro/NouPro.entitlements:6` | — | **OWNER** — verify on a real build |
| `MOB-20` | 🟡 | Version drift: `app.config.ts` says `buildNumber: '2'`/`versionCode: 2`, the native files say `1`/`1`. Possibly moot — `eas.json` sets no `appVersionSource`, so EAS CLI ≥18 defaults to `remote` and overrides both | `Info.plist:41`, `android/app/build.gradle:95` | — | **OWNER** — `eas build:version:get` |
| `MOB-21` | 🟡 | `app.json` is dead weight — `app.config.ts` spreads it then overrides every key it defines, and app.json's copy is staler. Editing it accomplishes nothing | `app.json` | — | OPEN |
| `OPS-7` | 🟡 | `npm run ios`/`android` run prebuild locally and re-merge `app.config.ts` into the committed native dirs, so hand-edits Expo re-asserts come back silently. `MOB-8`'s network config has no Expo equivalent and would vanish on `prebuild --clean` | `package.json:6`-`7` | — | OPEN (documented) |
...
| `MOB-1` | 🟠 | Tokens could be written to plaintext storage; SecureStore failures silently swallowed | `src/shared/store/profileStore.ts:19` | C1 | **FIXED** (2026-08-10) |
| ~~`MOB-2`~~ | — | ~~Biometric login never fires~~ | `App.tsx:939` | — | **WITHDRAWN — false finding** |
| `MOB-3` | 🟠 | OTA updates ship unsigned | `app.config.ts:153` | D | OPEN |
| `MOB-4` | 🟠 | `allowBackup="true"` — the plaintext store is swept into device/cloud backups | `AndroidManifest.xml:23` | D | OPEN |
| `MOB-5` | 🟠 | `SYSTEM_ALERT_WINDOW` in the release manifest | `AndroidManifest.xml:10` | D | OPEN |
| `MOB-6` | 🟠 | Payment WebView acted on `postMessage` from ANY origin, including a 3DS bank page | `PeachCheckoutWebView.tsx:28` | C1 | **FIXED** (2026-08-10) |
...
| `MOB-4` | `android:allowBackup="true"`, so the plaintext AsyncStorage store is swept into cloud/adb backups. **Correction:** this doc previously claimed the referenced `@xml/secure_store_*` rule files are missing and the build is therefore broken. That was wrong — they resolve from `expo-secure-store`'s own library resources via resource merging. `allowBackup` is the whole finding. The PII half is now closed by `MOB-14` | `AndroidManifest.xml:23` | `allowBackup="false"` |
...
|---|---|---|---|
| `AUTH-6` | ✅ **FIXED 2026-08-10 (E).** Register leaked account existence per identifier | `server.js:1737`-`1746` | Uniform 409 |
| `AUTH-7` | ✅ **FIXED 2026-08-10 (E).** Case-sensitive email lookup | `userRepo.prisma.js:19`-`23` | Normalize on write and lookup; add a functional unique index |
...
| `AUTH-11` | ✅ **FIXED 2026-08-10 (E).** Session + reset-token pruning wired into the scheduler | `sessionService.js` | Add to the automation cron |
...
| `AUTH-13` | ✅ **FIXED 2026-08-10 (E).** `jwt.verify` now pins `algorithms` | `auth.js:42` | `{ algorithms: ['HS256'] }`, plus `issuer`/`audience` |
...
| `ABUSE-7` | ✅ **FIXED 2026-08-10 (E).** `/api/products` rate-limited; filters pushed into the query | `server.js:16305` | Add `publicReadLimiter`, push filtering + pagination into the query |
| `TEN-4` | ✅ **FIXED 2026-08-10 (E).** Price-list item delete was scoped to the list but deleted by bare item id | `server.js:6446`-`6463` | Scope `removeItem` by both ids |
| `TEN-5` | ✅ **FIXED 2026-08-10 (E).** Location stock write never checked the product belonged to the location's business | `server.js:8353`-`8384` | Assert `product.businessId === locationBusinessId` |
| `TEN-6` | ✅ **FIXED 2026-08-10 (E).** `checkout-result` had no ownership check | `server.js:16931` | Verify `payment.businessId` membership |
| `EXP-3` | ✅ **FIXED 2026-08-10 (E).** Education/certifications/skills bypassed the block gate (`/experiences` was already fixed) | `server.js:6691`, `:6786`, `:6869`, `:6965` | Apply the same block check |
...
| `EXP-5` | ✅ **FIXED 2026-08-10 (E).** Cross-tenant discount validation returned the full record, unthrottled — a redemption oracle | `server.js:6150` | Scope to a legitimate customer relationship; throttle; return a boolean |
| `EXP-6` | ✅ **FIXED 2026-08-10 (E).** Opportunity/event detail ignored the status filter the list routes apply | `server.js:3368`, `:3534` | Gate on published/active state |
| `EXP-7` | ✅ **FIXED 2026-08-10 (E).** Storage error details now surface outside production only | `server.js:14844`-`14845` | Log detail, return generic + correlation id |
...
| `INJ-3` | ✅ **FIXED 2026-08-10 (E).** NaN/negative guards on qtyOnHand and invoice numerics | `server.js:8360`, `:15684`, `:10870` | `Number.isFinite` + range clamps |
...
| `INJ-5` | ✅ **FIXED 2026-08-10 (E).** Invoice totals now derived from line items at all three write paths | `server.js:10855`-`10882`, `:11123`-`11126`; `CreateInvoiceScreen.tsx:536`-`549` | Always recompute server-side; reject mismatches |
| `INJ-6` | ✅ **FIXED 2026-08-10 (E).** Procurement totals now derived from line items | `server.js:8731`, `:9033` | Recompute server-side |
...
| `INJ-8` | ✅ **FIXED 2026-08-10 (E).** Upload type now verified by magic bytes, not the client header | `server.js:545` | Sniff magic bytes, reject on mismatch |
| `INJ-9` | ✅ **FIXED 2026-08-10 (E).** Multer errors now return 4xx instead of a generic 500 | `server.js:14819` | Map `MulterError` → 400 |
...
| `OPS-2` | 🟡 **Client FIXED 2026-08-10 (C1); backend still OPEN.** No `beforeSend`/`beforeBreadcrumb`/`sendDefaultPii` on either end. Client now strips request bodies, cookies and auth headers, drops console breadcrumbs and scrubs ids from URLs. **The backend `Sentry.init` is unchanged** | `App.tsx:990` done · `server.js:41`-`46` open | Mirror the client's scrubbing on the backend |
...
| `MOB-11` | ✅ **FIXED 2026-08-10 (C1).** 2FA secret and backup codes went to the clipboard with no expiry — and "Copy & Close" was the ONLY way to dismiss the dialog | `TwoFactorAuthScreen.tsx:137` | Done: separate Copy/Close buttons, 90s auto-clear that compares before wiping |
| `MOB-12` | 🟠 **Not hygiene — a live broken feature.** The `noupro://` scheme is registered on NEITHER platform, yet `backend/public/join.html` and `product.html` hand users `noupro://` links and `App.tsx` lists it as a deep-link prefix. Those links do nothing today. The dev-client scheme `exp+noupro` ships in production instead | `Info.plist:25`-`39`, `AndroidManifest.xml:39`-`44`, `join.html:51` | Register `noupro` on both platforms |
| `MOB-13` | ✅ **FIXED 2026-08-10 (C1).** Per-account caches survived logout, so the next person on a shared device inherited them | `profileStore.ts:441` | Done: push token, avatar colours and the profile-picture cache (map + files) cleared. Order-status/pricing caches deliberately kept — server config with offline fallbacks |
| `MOB-17` | iOS requests **background** location (`NSLocationAlways*`) which the app cannot use — there is no `UIBackgroundModes` and no `ACCESS_BACKGROUND_LOCATION` — and **six of nine usage strings are raw `$(PRODUCT_NAME)` placeholders**. None of the reviewed copy in `app.config.ts` ever reached the binary: it lives in plugin options that only prebuild consumes. App Review rejection risk | `Info.plist:61`-`78` | Copy the real strings into the plist AND delete `locationAlwaysAndWhenInUsePermission` from `app.config.ts`, or a prebuild re-adds the Always keys. *(Renumbered from MOB-14, which now denotes the cached-PII fix.)* |
...
| **C0 — Client regressions** ✅ | Send `currentPassword`, adopt returned tokens, stop the 401 retry loop | **OTA** — published 2026-08-10 | none |
| **C1 — Client, JS only** ✅ | `MOB-1`, `MOB-6`, `MOB-11`, `MOB-13`, `MOB-14`, `OPS-2` (client half) | **OTA** `eas update --branch production` | none |
| **D — Client, native** | `MOB-3`, `MOB-4`, `MOB-5`, `MOB-8`, `MOB-9`, `MOB-12`, `MOB-16`, `MOB-17` | **`eas build`** + store submission | none |
| **D-deferred** | `MOB-10` (needs `npx expo install expo-screen-capture` first) | `eas build` | none |
...
| 2026-08-10 | F | Cross-tenant probes + route inventory guard | `27cdb01b` | Exported the app so HTTP-level tests are possible at all. Uses Node's global fetch and in-memory fakes — neither supertest nor Postgres is installable here — so it covers the middleware/membership/scoping logic where all six TEN findings lived, not Prisma's query semantics. **Both guards were verified to fail on a real bug** (reverting TEN-2, and appending a public route) rather than merely passing. |
| 2026-08-10 | E part 2 | `INJ-5`, `INJ-6` | `d095ee2a` | Money totals derived server-side, logging divergence rather than rejecting. Extracted to `src/domain/moneyTotals.js` so it could be tested. **Two live bugs fixed alongside**: the reprice formula was silently erasing shipping, and a zero-value PO stored `null`. |
| 2026-08-10 | E part 1 | `TEN-4`-`6`, `EXP-3`/`5`/`6`/`7`, `AUTH-6`/`7`/`11`/`13`, `INJ-3`/`8`/`9`, `ABUSE-7`, `OPS-2` (backend) | `fcaa8d57` | Also fixed a correctness bug the parallel work stream introduced: a `take: 500` cap on `/api/products` ran before the JS filtering, so `?companyId=` returned empty for anyone outside the newest 500 products. |
| 2026-08-10 | C1 | `MOB-1`, `MOB-6`, `MOB-11`, `MOB-13`, `MOB-14`, `OPS-2` (client) | `69052f61` | Client hardening, published by OTA. **Two findings were wrong and are corrected in this doc**: `MOB-2` withdrawn (biometric gate already existed — recon reported an already-fixed bug and I recorded it unverified), and `MOB-1`'s mechanism was misdescribed (the fallback was unreachable; the real bug was ten swallowed `.catch(() => {})`). `MOB-6` fixed by gating the message bridge on origin rather than the navigation allowlist the doc proposed — a nav allowlist would have broken 3-D Secure. |
| 2026-08-10 | C0 | Client regressions from B1 | `951269bf` | **Self-inflicted.** B1 made `currentPassword` required on four endpoints without updating the client, breaking email/phone change and 2FA setup in production. Also adopted the returned token pair (otherwise a silent sign-out ~30 min later) and bounded a pre-existing infinite refresh→replay loop on any 401 a refresh cannot fix. |
