...
const ProductCard: React.FC<ProductCardProps> = ({ 
  ...
  // Resolve listed state from multiple possible fields
  const isProductListed = product.isListed ?? product.is_listed ?? product.isDisplayable ?? false;

  // Handle listed toggle - delegates to parent for paywall check
  const handlePublishToggle = () => {
    if (onPublishToggle) {
      onPublishToggle(product.id, isProductListed);
    } else {
      // Fallback to direct update if no handler provided
      onUpdate?.('isListed', !isProductListed);
    }
  };
          ...
          <View style={styles.statusAndListedColumnEditing}>
            ...
            {showPublishToggle && (
              <View style={styles.publishSectionEditing}>
                <Label style={[
                  styles.publishLabel,
                  { color: isProductListed ? appTheme.colors.success : appTheme.colors.textMuted }
                ]}>
