...
const businessTypes: DropdownItem[] = [
  { id: 'general_retail', title: 'Retail & Distribution', icon: 'storefront-outline' },
  { id: 'food_beverage', title: 'Food & Beverage', icon: 'restaurant-outline' },
  { id: 'production', title: 'Production & Manufacturing', icon: 'construct-outline' },
  { id: 'services', title: 'Services', icon: 'hammer-outline' },
  { id: 'cosmetics', title: 'Cosmetics & Beauty', icon: 'sparkles-outline' },
  { id: 'electronics', title: 'Electronics & Tech', icon: 'laptop-outline' },
  { id: 'other', title: 'Other', icon: 'business-outline' },
];
...
export default function CompanyEditScreen() {
  ...
  let parsedCountryCode = '+230';
  let parsedPhoneNumber = '';
  if (rawPhone) {
    if (rawPhone.startsWith('+230')) {
      parsedCountryCode = '+230';
      parsedPhoneNumber = rawPhone.slice(4);
    } else if (rawPhone.startsWith('+')) {
      const match = rawPhone.match(/^(\+\d{2,3})(.+)$/);
      if (match) {
        parsedCountryCode = match[1];
        parsedPhoneNumber = match[2];
      } else {
        parsedPhoneNumber = rawPhone;
      }
    } else {
      parsedPhoneNumber = rawPhone;
    }
  ...
  // Default business hours used as fallback
  ...
  const initialBusinessType = currentCompany?.industry || '';
  const initialBusinessCategory = currentCompany?.category || '';
  const initialEmail = currentCompany?.email || '';
  const initialWebsite = currentCompany?.website || '';
  const initialCountryCode = parsedCountryCode;
  const initialPhoneNumber = parsedPhoneNumber;
  // Get address from current company
  const initialAddress = currentCompany?.address || '';
  const initialBusinessHours: BusinessHour[] = (currentCompany?.settings?.businessHours as BusinessHour[] | undefined) || defaultBusinessHours;

  // Invoice Settings initial values
  const initialCurrency = currentCompany?.settings?.currency || 'USD';
  const initialTaxRate = (currentCompany?.settings?.taxRate || 0).toString();
  const initialInvoicePrefix = currentCompany?.settings?.invoicePrefix || 'INV';
  const initialAllowPartialPayments = currentCompany?.settings?.allowPartialPayments ?? true;
  const initialAutoGenerateInvoices = currentCompany?.settings?.autoGenerateInvoices ?? false;
  ...
  }, [coverImage, profileImage, companyName, businessDescription, businessType, businessCategory, email, website, countryCode, phoneNumber, address, businessHours, currency, taxRate, invoicePrefix, allowPartialPayments, autoGenerateInvoices]);
        ...
        {isSaving ? (
          ...
          <Text
            style={[
              ...
              { color: (hasChanges && !isSaving) ? '#FFFFFF' : appTheme.colors.textMuted },
            ]}
          >
            Save Changes
          </Text>
          ...
          {BUSINESS_CATEGORIES.map((cat, index) => (
            <ListItemCard
              ...
              avatar={{
                type: 'icon',
                icon: cat.icon,
                iconColor: businessCategory === cat.id ? appTheme.colors.primary : appTheme.colors.text,
                backgroundColor: appTheme.colors.surface,
              }}
              title={cat.label}
              onPress={() => {
              ...
              selected={businessCategory === cat.id}
              showCheckmark
